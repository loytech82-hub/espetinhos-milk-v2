-- =============================================
-- ESPETINHOS MILK - Setup do Banco de Dados
-- Execute este SQL no Supabase Dashboard > SQL Editor
-- =============================================

-- TABELA: profiles (perfis de usuario com roles)
...
-- TABELA: categorias (normalizar categorias de produtos)
...
-- TABELA: caixa_turnos (abrir/fechar caixa)
...
-- TABELA: estoque_movimentos (historico de estoque)
CREATE TABLE IF NOT EXISTS public.estoque_movimentos (
  ...
  comanda_id INTEGER REFERENCES public.comandas(id),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Adicionar campo turno_id na tabela caixa (se nao existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'caixa' AND column_name = 'turno_id'
  ) THEN
    ALTER TABLE public.caixa ADD COLUMN turno_id INTEGER REFERENCES public.caixa_turnos(id);
  END IF;
END $$;

-- Adicionar campo usuario_id na tabela caixa (se nao existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'caixa' AND column_name = 'usuario_id'
  ) THEN
    ALTER TABLE public.caixa ADD COLUMN usuario_id UUID REFERENCES auth.users(id);
  END IF;
END $$;

-- Adicionar campo usuario_id na tabela comandas (se nao existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'comandas' AND column_name = 'usuario_id'
  ) THEN
